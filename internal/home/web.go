...
type webAPIConfig struct {
	...
	// firstRun, if true, tells ADBlock-Private-DNS to register install handlers.
	firstRun bool

	// disableUpdate, if true, tells ADBlock-Private-DNS to not check for updates.
	disableUpdate bool
	...
	// serveHTTP3, if true, tells ADBlock-Private-DNS to start HTTP3 server.
	serveHTTP3 bool
}
...
func newWebAPI(ctx context.Context, conf *webAPIConfig) (w *webAPI) {
	...
	if conf.firstRun {
		conf.logger.InfoContext(
			ctx,
			"This is the first launch of ADBlock-Private-DNS, redirecting everything to /install.html",
		)
...
func (web *webAPI) start(ctx context.Context) {
	...
	web.logger.InfoContext(ctx, "ADBlock-Private-DNS is available at the following addresses:")
